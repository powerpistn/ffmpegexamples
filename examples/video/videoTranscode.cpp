...
void doTranscode() {
    ...
    do {
        ...
        string dstFile= "/testffmpeg/big_buck_bunny.mov";
        ...
        std::cout<<"7.输出文件头文件信息写入成功，输出文件信息查看成功"<<std::endl;
        ...
        std::cout<<"8.获取视频帧Frame数据缓存区成功，开始判断是否需要视频流参数是否需要转换"<<std::endl;
        //判断是否需要进行转换
        ...
        std::cout<<"9.获取视频帧Frame数据缓存区成功，开始判断是否需要视频流参数是否需要转换"<<std::endl;
        //判断是否需要进行转换
        ...
        std::cout<<"9.源文件读取数据开始转码"<<std::endl;
        //从源文件中读取数据
        ...
        avcodec_send_packet(srcVideoCodecCtx, NULL);
        avcodec_send_frame(dstVideoCodecCtx, NULL);
        avcodec_send_packet(srcAudioCodecCtx, NULL);

        std::cout<<"11.刷新音视频缓冲"<<std::endl;
        if (audioSwrCtx) {
        ...
        std::cout<<"12.向输出文件写入尾部标识"<<std::endl;
        //向文件中写入文件尾部标识，并释放该文件
        av_write_trailer(dstFmtCtx);
    } while (0);

    std::cout<<"13.释放资源"<<std::endl;
    //释放资源
    releaseRes();
}
...
//视频模块初始化：主要为输出文件添加视频流，并为之后的视频流解码与编码设置参数
//返回0表示成功，返回一个负数表示失败
int addVideoStream() {
    int ret = 0;
